...
from .core import diagnose_unit, list_failed_units, CAUSE_CLEAN_NOT_CRASHING, CAUSE_UNKNOWN, CAUSE_DIAGNOSTIC_FAILED, CAUSE_UNIT_NOT_FOUND
...
def _print_text(reports, style) -> None:
    for r in reports:
        level = "ok" if r.cause == CAUSE_CLEAN_NOT_CRASHING else ("warn" if r.cause in (CAUSE_DIAGNOSTIC_FAILED, CAUSE_UNIT_NOT_FOUND) else "fail")
        print()
